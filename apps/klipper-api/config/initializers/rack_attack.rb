class Rack::Attack
  # Throttle sign_in/sign_up/password_resets attempts by IP: at most 20 per minute.
  throttle("auth/ip", limit: 20, period: 1.minute) do |req|
    req.ip if req.post? && %w[/api/v1/auth/sign_in /api/v1/auth/sign_up /api/v1/password_resets].include?(req.path)
  end
