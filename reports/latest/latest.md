...
**Trigger:** workflow_run · **Branch:** main · **Commit:** `0593a8efe386b0a598ffae5ecab85d18d4dd7a65` · **Modo:** fast
...
|---|---|---|---|---|
| FIN-004 | 🔴 critical | finance | 576 | O hero do dashboard ainda calcula spentRatio inline com fallback para 0 em vez de reusar pctOfIncome() (já disponível no mesmo composable e já usado duas linhas abaixo), então um ciclo sem receita nenhuma é desenhado como '0% gasto, dentro da faixa' em vez de 'sem dado' — o mesmo defeito catalogado como FIN-004 continua sem correção. |
...
| ARCH-005 | 🟡 medium | architecture | 45 | Quatro páginas centrais do Wealth OS (dashboard, contas, orçamento, portadores) continuam sem nenhum teste de página, embora investimentos já tenha sido corrigido desde a última auditoria. |
...
| SEC-003 | 🟢 low | security | 10.8 | `devalue` (nova advisory moderate desde a última auditoria) e `esbuild` (low, recorrente) seguem desatualizados em devDependencies transitivas de ambos os frontends; o lado Ruby não pôde ser verificado neste ambiente por falta de bundler instalado. |
...
| SEC-006 | 🟢 low | security | 2 | O access token da SPA continua exposto a JavaScript via cookie não-httpOnly, e a decisão arquitetural de aceitar esse risco (mitigado pela CSP com nonce) ou migrar para cookie HttpOnly + CSRF segue sem resolução formal. |
