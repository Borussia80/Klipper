...
<div style="padding:32px 0 0">
  <div style="display:flex;align-items:center;gap:14px;margin-bottom:6px">
    <div style="width:52px;height:52px;display:flex;align-items:center;justify-content:center;
      ...
      color:var(--ink)">
      {BRAND_SVG.replace('width="22"', 'width="28"').replace('height="22"', 'height="28"')}
    </div>
    <div>
  ...
  <div style="font-family:'Instrument Serif',Georgia,serif;font-size:16px;color:var(--ink-3);
    margin:18px 0 32px;line-height:1.5;max-width:560px">
...
st.markdown(
    f'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-bottom:40px">{cards_html}</div>',
    unsafe_allow_html=True,
)
...
engines_html = "".join(
    f"""<div style="display:grid;grid-template-columns:28px 120px 1fr;align-items:center;gap:12px;
...
<div class="k-card gilt" style="margin-bottom:28px">
  <div class="k-card-h">
    <div>
      ...
      <div style="font-family:var(--font-sans);font-size:11.5px;color:var(--ink-3);margin-top:2px">engines M · decisão ancorada em matemática</div>
    </div>
  </div>
